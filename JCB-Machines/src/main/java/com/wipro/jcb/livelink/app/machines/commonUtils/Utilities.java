...
public class Utilities {
    ...
    public String updateLivelinkServerToken(User user, Boolean isForcefullUpdate, String userDetails) throws Exception {
		try {
				...
				synchronized (userToken) {
					...
					if (isForcefullUpdate || userToken.getExpiresAt() != 0) {
						if (userToken.getExpiresAt() < now + tokenRenewTime) {
							...
							ResponseEntity<String> response = webClientBuilder.build().get().uri(userUri + "/api/user/getPwdByUserName")
					        		.header(MessagesList.LoggedInUserRole, userDetails).retrieve().toEntity(String.class).block();
